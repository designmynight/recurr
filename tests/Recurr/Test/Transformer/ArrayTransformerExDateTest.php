...
class ArrayTransformerExDateTest extends ArrayTransformerBase
{
    ...
    public function testExDateNoTime()
    {
        $rule = new Rule(
            ...
            new Carbon('2014-06-01')
        );

        $computed = $this->transformer->transform($rule);

        $this->assertCount(2, $computed);
        $this->assertEquals(new Carbon('2014-06-01'), $computed[0]->getStart());
        $this->assertEquals(new Carbon('2014-06-03'), $computed[1]->getStart());
    }
    ...
    public function testExDateWithTime()
    {
        $rule = new Rule(
            ...
            new Carbon('2014-06-01 04:00:00')
        );

        $computed = $this->transformer->transform($rule);

        $this->assertCount(2, $computed);
        $this->assertEquals(new Carbon('2014-06-01 04:00:00'), $computed[0]->getStart());
        $this->assertEquals(new Carbon('2014-06-02 04:00:00'), $computed[1]->getStart());
    }
    ...
    public function testExDateWithUtcTime()
    {
        $rule = new Rule(
            ...
            new Carbon('2014-06-01 07:00:00')
        );

        $computed = $this->transformer->transform($rule);

        $this->assertCount(2, $computed);
        $this->assertEquals(new Carbon('2014-06-01 07:00:00'), $computed[0]->getStart());
        $this->assertEquals(new Carbon('2014-06-03 07:00:00'), $computed[1]->getStart());
    }
    ...
    public function testExDateWithMixedTimezones()
    {
        ...
        $this->assertEquals(new Carbon('2014-06-03 04:00:00'), $computed[0]->getStart());
    }
    ...
    public function testSetExDates()
    {
        ...
        $exDates = array(
            new DateExclusion(new Carbon('2014-06-02'), false),
        );
